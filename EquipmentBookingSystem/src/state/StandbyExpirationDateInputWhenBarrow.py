...
class StandbyExpirationDateInputWhenBarrow(state.IState):
    ...
    def exit(self):
        ...
        result = dbregist.DBregistration_Borrow(state.CommonResource.equipmentId,state.CommonResource.employeeId,return_date)
        if result == True:
            ...
            self.__get_next_state.set_next_state(state.SuccessBarrowEquipment())
        else:
