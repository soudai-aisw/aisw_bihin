...
# pathlib.Pathで相対パスを絶対パスに変換したかったがうまく動かなかったので代替手段で実装
# ひとまず「src」フォルダ名で切る。
spfldpath = filepath.split("src")
filepath_user = os.path.join(spfldpath[0],"database","db_userlist.csv")
filepath_item = os.path.join(spfldpath[0],"database","db_itemlist.csv")
...
def DBregistration_EmpIDtoEmpNo(EmpID,EmpNo):
    ...
    if len(df_tgt) == 1:
        ...
        df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('備考')] = EmpID
        df_csv.to_csv(filepath_user, index=False, header=True, encoding="cp932", mode='w')

        # バックアップを出力
        # タイムスタンプは「年月日_時分秒」で取得
        add_time = datetime.now().strftime('%Y%m%d_%H%M%S_')
        bkupfilepath = filepath_user.replace(os.path.basename(filepath_user),"BKUP_" + add_time + os.path.basename(filepath_user))
        df_csv.to_csv(bkupfilepath, index=False, header=True, encoding="cp932", mode='w')
...
def DBregistration_Borrow(EquID,EmpNo,RetDay):
    ...
    if len(df_tgt) == 1:
        ...
        if df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('内容')] == "借用可能":
            # 書き戻すデータは「-」埋めしてないデータにする。
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('ユーザID')] = EmpNo
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('開始日付')] = brw_day
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('終了日付')] = RetDay
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('内容')] = "借用中"
            df_csv.to_csv(filepath_item, index=False, header=True, encoding="cp932", mode='w')

            # バックアップを出力
            # タイムスタンプは「年月日_時分秒」で取得
            add_time = datetime.now().strftime('%Y%m%d_%H%M%S_')
            bkupfilepath = filepath_item.replace(os.path.basename(filepath_item),"BKUP_" + add_time + os.path.basename(filepath_item))
            df_csv.to_csv(bkupfilepath, index=False, header=True, encoding="cp932", mode='w')
...
def DBregistration_Return(EquID):
    ...
    if len(df_tgt) == 1:
        ...
        if df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('内容')] == "借用中":
            # 書き戻すデータは「-」埋めしてないデータにする。
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('ユーザID')] = "-"
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('開始日付')] = "-"
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('終了日付')] = "-"
            df_csv.iat[df_tgt.index.values[0],df_tgt.columns.get_loc('内容')] = "借用可能"
            df_csv.to_csv(filepath_item, index=False, header=True, encoding="cp932", mode='w')
